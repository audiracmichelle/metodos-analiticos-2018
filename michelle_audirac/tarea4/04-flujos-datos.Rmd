...
- Un conjunto $\Omega$ de posibles valores (el universo) que puede aparecer en el flujo
...
```{r}
palabras_prueba <- c('mezcal', 'menso', 'hipster', 'cholo')
df_palabras <- data_frame(palabra = palabras_prueba) %>%
                   mutate(pertenece = map_lgl(palabra, bloom_filter$in_filter))
df_palabras
```
...
```{r}
generar_dist_1('chola') %>% keep(bloom_filter$in_filter)
```

```{r}
generar_dist_1('sholo') %>% keep(bloom_filter$in_filter)
```
...
*caso 1 no se elige el elemento n
$k/n_0$ la probabilidad de cada elemento del flujo inicial
por
$1-k/n$ la probabilidad de que no sea seleccionado el nuevo elemento

más

*caso 2 se elige el elemento n
$k/n_0$ la probabilidad de cada elemento del flujo inicial
por 
$k/n$ la probabilidad de que sea seleccionado el nuevo elemento
por 
$1-1/k$ la probabilidad de no ser eliminado

si n_0 = n-1

$$
(k/n_0)(1-k/n) + (k/n_0)(k/n)(1-1/k) = \\
\frac{k(n-k)}{n_0n} + \frac{k(k-1)}{n_0n} = \\
\frac{k(n-1)}{n_0n} = k/n
$$

por inducción hacia adelante
...
```{r}
library(sde)
serie <- BM(x = rnorm(1), T = 1000, N = 1000)


prom_c <- prom_exponencial(serie[1], 0.05)
serie_pexp <- map_dbl(serie, prom_c)

plot(serie)
lines(serie_pexp, col = "red")
```
