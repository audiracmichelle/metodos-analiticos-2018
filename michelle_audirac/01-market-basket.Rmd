...
**Observación**:
  Si hay $m$ artículos, entonces el número de posibles itemsets es de $2^m$. El número
total de itemsets de tamaño $k$ es $\binom{m}{k}$. Sin embargo, esperamos que los itemsets frecuentes sean raros, de forma que es factible calcularlos (siempre y cuando el soporte mínimo sea suficientemente grande).
...
conjuntos frecuentes de soporte mínimo 0.005 (como hay alrededor de 10000 canastas, esto significa que canastas que aparecieron al menos 50 veces durante el mes):
...
2. Por el principio de monotonicidad, ningún par frecuente puede contener un artículo no frecuente. Así que para calcular pares:
...
paper original sugiere una estructura de árbol para encontrar los subconjuntos de $t$ que están en $C_k$. Ver también [@borgelt].
...
en términos de reglas de asociación. Un ejemplo de una regla de asociación es:
